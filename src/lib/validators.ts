...
export function validateAbsenceCreation(startDate: string): AbsenceBufferResult {
  ...
  if (diffHours < 24) {
    return { valid: true, warning: 'La inasistencia se está registrando con menos de 24 h de anticipación' };
  }
  return { valid: true };
}
