...
import { useTeacherPublicAbsences, TeacherPublicAbsence } from '../hooks/queries';
...
export const DocentePublico: React.FC<DocentePublicoProps> = ({ level }) => {
  ...
  const { data = [], isLoading } = useTeacherPublicAbsences(month, year, level);
      ...
      <PageHeader
        ...
        filters={
          <>
            ...
            <Select options={years} value={year} onChange={(e) => setCurrentDate(new Date(Number(e.target.value), month, 1))} className="md:w-36" />
          </>
        }
      />
        ...
        {!selected ? null : (
          <div className="space-y-5">
            ...
            <div>
              ...
              {selected.affected_tests_json?.length ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {selected.affected_tests_json.map((test) => (
