...
export const Inasistencias: React.FC<InasistenciasProps> = ({ level }) => {
  ...
  const onUpdate = async (data: Partial<Omit<import('../types').Absence, 'id' | 'created_at'>>) => {
    ...
    try {
      ...
      showToast({ type: 'success', message: 'Inasistencia actualizada exitosamente' });
      if (file && res && !res.uploadFailed && res.row && res.row.document_url) {
        showToast({ type: 'success', message: `Documento ${file.name} subido: ${res.row.document_url}` });
      }
      if (res && res.uploadFailed) {
        showToast({ type: 'warning', message: 'La inasistencia se guardó, pero NO se pudo subir el documento. Puedes reintentar adjuntarlo desde el detalle.' });
      }
    } catch (error) {
      console.error('Error updating absence:', error);
      showToast({ type: 'error', message: 'Error al actualizar la inasistencia' });
    } finally {
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      showToast({ type: 'success', message: 'Inasistencia registrada exitosamente' });
      if (file && res && !res.uploadFailed && res.row && res.row.document_url) {
        showToast({ type: 'success', message: `Documento ${file.name} subido: ${res.row.document_url}` });
      }
      if (res && res.uploadFailed) {
        showToast({ type: 'warning', message: 'La inasistencia se registró, pero no se pudo subir el documento. Intenta nuevamente.' });
      }
    } catch (error) {
      console.error('Error creating absence:', error);
      showToast({ type: 'error', message: 'Error al registrar la inasistencia' });
    } finally {
      patchUiState({ mutationLoading: false });
    }
  };
